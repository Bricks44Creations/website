...
// ======== FILTRE PAR FILM / SÉRIE MARVEL ========
// ======== FILTRE MARVEL — MULTI-SÉLECTION À PARTIR DU DOM ========
const sortMarvelBtn = document.getElementById("sort-marvel-btn");
const sortMarvelLabel = document.getElementById("sort-marvel-label");
const sortMarvelArrow = sortMarvelBtn?.querySelector(".arrow-down");
const sortMarvelOptions = document.getElementById("sort-marvel-options");

const selectedMarvel = new Set(); // titres cochés

function getAvailableMarvelTitles() {
  const set = new Set();
  document.querySelectorAll(".portfolio-card").forEach(card => {
    const v = (card.dataset.marvel || "").trim();
    if (v) set.add(v);
  });
...
function updateMarvelLabel() {
  if (!sortMarvelLabel) return;
  if (selectedMarvel.size === 0) {
    sortMarvelLabel.setAttribute("data-key", "filter_all_label");
    sortMarvelLabel.textContent = t("filter_all_label");
    return;
  }
  const vals = Array.from(selectedMarvel);
  const preview = vals.slice(0, 2).join(", ");
  const extra = vals.length > 2 ? ` +${vals.length - 2}` : "";
  sortMarvelLabel.removeAttribute("data-key");
  sortMarvelLabel.textContent = `${t("filter_by_prefix")}${preview}${extra}`;
}
  // Affiche 1–2 valeurs, puis “+N”
  const vals = Array.from(selectedMarvel);
  const preview = vals.slice(0, 2).join(", ");
  const extra = vals.length > 2 ? ` +${vals.length - 2}` : "";
  sortMarvelLabel.textContent = `Filtrer par : ${preview}${extra}`;
}

function applyMarvelFilterMulti() {
  const cards = document.querySelectorAll(".portfolio-card");
  const showAll = selectedMarvel.size === 0;

  cards.forEach(card => {
    const tag = (card.dataset.marvel || "").trim();
    const visible = showAll || selectedMarvel.has(tag);
    card.style.display = visible ? "" : "none";
  });

  // Conserver l’ordre choisi (nom/date/pertinence)
  sortPortfolio(); // déjà défini dans script.js:contentReference[oaicite:7]{index=7}
  updateMarvelLabel();
}

function buildMarvelMenuMulti() {
  if (!sortMarvelOptions) return;
  sortMarvelOptions.innerHTML = "";

  // Ligne "Tous" (efface la sélection)
  const allRow = document.createElement("label");
  allRow.className = "checkbox-item";
  const allCb = document.createElement("input");
  allCb.type = "checkbox";
  allCb.checked = selectedMarvel.size === 0;
  allCb.addEventListener("change", (e) => {
    if (e.target.checked) {
      selectedMarvel.clear();
      // décocher les autres
      sortMarvelOptions.querySelectorAll('input[type="checkbox"].marvel-opt').forEach(cb => {
        cb.checked = false;
      });
      applyMarvelFilterMulti();
    } else {
      // empêcher d'avoir zéro case cochée si on décoche "Tous" sans rien d'autre
      if (selectedMarvel.size === 0) {
  ...
  allText.textContent = t("filter_all");
  allRow.appendChild(allCb);
  allRow.appendChild(allText);
  sortMarvelOptions.appendChild(allRow);
  ...
  sortMarvelOptions.appendChild(hr);

  // Titres disponibles depuis la page
  getAvailableMarvelTitles().forEach(title => {
    const row = document.createElement("label");
    row.className = "checkbox-item";

    const cb = document.createElement("input");
    cb.type = "checkbox";
    cb.className = "marvel-opt";
    cb.dataset.value = title;
    cb.checked = selectedMarvel.has(title);

    cb.addEventListener("change", () => {
      // si on coche un titre, on décoche "Tous"
      sortMarvelOptions.querySelector('label .checkbox-item'); // no-op (compat)
      allCb.checked = false;
      if (cb.checked) selectedMarvel.add(title);
      else selectedMarvel.delete(title);

      // si plus rien n'est coché -> revient à "Tous"
      if (selectedMarvel.size === 0) {
        allCb.checked = true;
      }
      applyMarvelFilterMulti();
    ...
    row.appendChild(cb);
    row.appendChild(txt);
    sortMarvelOptions.appendChild(row);
  });

  // Actions facultatives (Appliquer / Effacer tout)
  const actions = document.createElement("div");
  actions.className = "actions";
  const clearBtn = document.createElement("button");
  clearBtn.type = "button";
  clearBtn.textContent = t("action_clear_all");
  clearBtn.addEventListener("click", () => {
    selectedMarvel.clear();
    sortMarvelOptions.querySelectorAll('input[type="checkbox"].marvel-opt').forEach(cb => cb.checked = false);
    allCb.checked = true;
    applyMarvelFilterMulti();
  });
  ...
  closeBtn.addEventListener("click", () => {
    sortMarvelOptions.classList.remove("show");
    sortMarvelArrow?.classList.remove("rotate");
  });

  actions.appendChild(clearBtn);
  actions.appendChild(closeBtn);
  sortMarvelOptions.appendChild(actions);
}

// Ouverture/fermeture du menu
sortMarvelBtn?.addEventListener("click", (e) => {
  e.stopPropagation();
  buildMarvelMenuMulti();
  sortMarvelOptions?.classList.toggle("show");
  // Position & size (CSS expects position:fixed on #sort-marvel-options)
  if (sortMarvelOptions && sortMarvelOptions.classList.contains("show")) {
    const rect = sortMarvelBtn.getBoundingClientRect();
    const margin = 8;
    const targetWidth = Math.min(Math.max(rect.width, 320), Math.floor(window.innerWidth * 0.9));
    const left = Math.min(rect.left, window.innerWidth - targetWidth - 8);
    const top = rect.bottom + margin;
    sortMarvelOptions.style.left = `${left}px`;
    sortMarvelOptions.style.top = `${top}px`;
    sortMarvelOptions.style.width = `${targetWidth}px`;
  }
  sortMarvelArrow?.classList.toggle("rotate", sortMarvelOptions?.classList.contains("show"));
});

window.addEventListener("click", (e) => {
  if (!e.target.closest("#sort-marvel-btn") && !e.target.closest("#sort-marvel-options")) {
    sortMarvelOptions?.classList.remove("show");
    sortMarvelArrow?.classList.remove("rotate");
  }
});

// Init : label correct au chargement
document.addEventListener("DOMContentLoaded", () => {
  updateMarvelLabel();
});
